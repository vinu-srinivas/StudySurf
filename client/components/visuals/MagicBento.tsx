...
const DEFAULT_GLOW_COLOR = '43, 0, 255'; // #2B00FF
...
function GlobalSpotlight({
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      ...
      gridRef.current?.querySelectorAll<HTMLElement>('.card').forEach((card) => card.style.setProperty('--glow-intensity', '0'));
...
function BentoCardGrid({ children, gridRef }: { children: React.ReactNode; gridRef: React.RefObject<HTMLDivElement> }) {
  return (
    <div className="bento-section grid gap-6 p-4 max-w-[72rem] select-none relative" style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }} ref={gridRef}>
      {children}
    </div>
  );
}
  ...
  return (
    <>
      <style>{`
        ...
        .card--border-glow::after { content: ''; position: absolute; inset: 0; padding: 6px; background: radial-gradient(var(--glow-radius) circle at var(--glow-x) var(--glow-y), rgba(${glowColor}, calc(var(--glow-intensity) * 0.8)) 0%, rgba(${glowColor}, calc(var(--glow-intensity) * 0.4)) 30%, transparent 60%); border-radius: inherit; mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); -webkit-mask-composite: xor; pointer-events: none; transition: opacity 0.3s ease; z-index: 1; }
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-6">
          {cards.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[280px] w-full max-w-full p-7 rounded-2xl border font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${
              enableBorderGlow ? 'card--border-glow' : ''
            }`;

            const cardStyle: CSSProperties & Record<string, any> = {
              backgroundColor: card.color || 'rgba(255,255,255,0.04)',
            ...
            return (
              <ParticleCard key={index} className={baseClassName} style={cardStyle} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                <div className="card__header relative text-white h-0" />
                <div className="card__content flex flex-col relative text-white">
                  <div className="mb-2 inline-flex h-10 w-10 items-center justify-center rounded-md bg-white/10 text-white/90">{card.icon}</div>
                  <h3 className={`card__title font-semibold text-base m-0 mb-1`}>{card.title}</h3>
                  <p className={`card__description text-[18px] leading-6 opacity-90`}>{card.description}</p>
                </div>
              </ParticleCard>
            );
          })}
